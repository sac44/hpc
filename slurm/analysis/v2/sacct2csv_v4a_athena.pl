...
my @tresfields = qw(
        ...
        AllocTRES_billing AllocTRES_cpu AllocTRES_energy AllocTRES_gpu AllocTRES_mem AllocTRES_node
);
...
my $field_string = join(",", @fields_query);
...
# Command to run
my $cmd = "sacct --allocations $array_option  --allusers --parsable2 --noheader --truncate --duplicates --noconvert -o $field_string --starttime=$sacct_start --endtime=$sacct_end";
...
while (my $line = <$sacct_fh>) {
    ...
    if ($debug) { for my $i (0 .. $#fields) {print "[DEBUG] $fields[$i] = $row_values[$i]\n";} };

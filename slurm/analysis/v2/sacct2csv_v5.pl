...
while (my $line = <$sacct_fh>) {
	...
	if ($debug) {print Dumper %tres_details;};
    }
    ...
    my @extended_values = ($period_id, $period_duration, $cluster_name, $timestamp->strftime("%Y-%m-%dT%H:%M:%S"), @row_values, map { $tres_details{$_} // '' } @tresfields);

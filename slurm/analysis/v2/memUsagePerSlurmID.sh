...
for YEAR in $(seq $startYear $thisyear)
do
  ...
  sacct -n --array -a --format JobID,NTasks,MaxRSS,MaxVMSize,REQMEM,AllocCPUS,NNodes  --noconvert -P --start=$YEAR-$MM-01 --end=$YEAR-$MM-31 | \
        ...
        awk -v cluster=$cluster -F'|' 'BEGIN {OFS="|"} {rss[$1] = (rss[$1]>($2*$3)) ? rss[$1] : ($2*$3); vmsize[$1] = (vmsize[$1]>($2*$4)) ? vmsize[$1] : ($2*$4); ntasks[$1] = (ntasks[$1]>$2 ? ntasks[$1] : $
2); reqmem[$1] = (reqmem[$1]>($5)) ? reqmem[$1] : ($5); ncpus[$1] = (ncpus[$1]>($6)) ? ncpus[$1] : ($6); nnodes[$1] = (nnodes[$1]>($7)) ? nnodes[$1] : ($7)} END {for (sid in rss) print cluster, sid, ntasks[s
id], rss[sid], reqmem[sid]*1024^2, vmsize[sid], ncpus[sid], nnodes[sid], rss[sid] / nnodes[sid], cluster"_"sid}' | sed -e 's/\(^[0-9]\+\)/\1\|/g' | sed 's/|_/|/g' | tee >> $logfile

...
foreach (@dirlist) {
   ...
   $total_size     += $size;
   $ave_size += ($size - $ave_size) / $nfiles_so_far;
   $total_cap_used += $cap_used;
   $ave_cap_used += ($cap_used - $ave_cap_used) / $nfiles_so_far;
   if($cap_used >= $size) {
...
sub print_report {
  ...
  printf($fh "funber of files so far,%d\n", $nfiles_so_far);
  printf($fh "total capacity used,%s\n", kb_to_print($total_cap_used));
  printf($fh "average capacity used,%s\n",kb_to_print($ave_cap_used));
  printf($fh "total user data,%s\n", kb_to_print($total_size));
  printf($fh "average file size so far,%s\n",kb_to_print($ave_size));
